  ...
  const handleFileUpload = () => {
    ...
    const result = exactData?.map((student) => {
      let newStudent = {
        ...
        roomNo: (student?.roomNo || "").toString().trim(),
        college: (student?.college || "").toString().trim().toUpperCase(),
        year: parseInt((student?.year || 0).toString().trim()) || 0,
        ...
        parentName: (student?.parentName || "").toString().trim(),
        parentPhoneNo: (student?.parentPhoneNo || "").toString().trim().replace(/[\r\n]+/gm, " "),
        hostelId: (student?.gender || "").toString().toUpperCase() === "MALE" ? "BH1" : "GH1",
        requestCount: 0,
